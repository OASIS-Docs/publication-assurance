...
This catalog records, for the acceptance-criteria checks that trace to **written OASIS policy**, exactly which policy each one answers to: the governing document, the section, and the verbatim clause. Of the 171 individual check conditions, **97 are grounded in written policy**, appearing below as 94 catalog entries because 3 check signatures each cover more than one condition (the markdown-source and HTML-render forms of one rule). The remaining 74 are operational quality rules earned from real publications, with no single written clause behind them; `crosswalk.json` lists them as such.
