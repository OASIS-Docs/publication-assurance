...
def check_schemas(stage_dir: str, base_this_stage: str, version: str, stage: str,
    ...
    for root, _dirs, files in os.walk(stage_dir):
        for name in files:
            ...
            blob = json.dumps(doc)
            for u in sorted(set(re.findall(r"https://docs\.oasis-open\.org/[^\"\s]+\.json", blob))):
...
def check_pdf(pdf_path: str, this_urls_base: str, version: str, f: Findings) -> None:
    ...
    if spec_base:
        for m in sorted(set(re.findall(re.escape(spec_base) + r"/v(\d+\.\d+)/", txt))):
            if f"v{m}" != version:
                f.add(WARN, "pdf-sync",
...
def check_correction_classes(md_text: str, stage_dir: str, base: str, stage: str,
    ...
    for m in sorted(set(re.findall(r"\]\((\.?/[^)\s]*//[^)\s]*)\)", prose))):
...
def emit_manifest_txt(stage_dir: str, version: str, stage: str,
    ...
    lines = [
        ...
        "(OASIS-Docs/publication-assurance)."
        + (" The machine-readable companion is manifest.json in this"
           " directory." if with_json else ""),
        "",
        "====================================",
...
def check_policy(md_text: str, html_text: str, version: str, stage: str,
    ...
    # canonical logo
    for m in sorted(set(re.findall(r"!\[[^\]]*\]\((\S*?[Ll]ogo\S*?)\)", md_text))):
    ...
    for u in sorted(set(re.findall(r"https://docs\.oasis-open\.org/\S+", prose))):
...
#
# "applies": all = every package; md, docx and odt = add-on conditions that
# engage only when the package carries that source format. "requires": a
# package/environment feature without which the condition is not evaluated
# (reported NA, never a fake PASS).
...
CONDITION_DOCS: list[dict] = [
    ...
    dict(check="version-naming", sig="the files were renamed", applies="all",
    ...
    dict(check="version-naming", sig="does not embed the version segment", applies="all",
    ...
    # revision-collision
    dict(check="revision-collision", sig="is already published at", applies="all", requires="network",
    ...
    # filenames
    dict(check="filenames", sig="No delivery items found", applies="all",
    ...
    dict(check="filenames", sig="No authoritative source artifact", applies="all",
    ...
    dict(check="stage-uri-live", sig="URI returns HTTP", applies="md", requires="network",
    ...
    dict(check="member-uri", sig="Cites an OASIS member-only (Kavi) URI", applies="all",
    ...
    # residue
    dict(check="residue", sig="Editor TODO left in", applies="all",
    ...
    dict(check="residue", sig="'tbd' placeholder section", applies="all",
         condition="No bare 'tbd' placeholder sections",
         pulls="prose of the markdown and HTML (code blocks stripped)",
         compares_to="no line consisting solely of 'tbd'"),
    dict(check="residue", sig="Template instruction left in", applies="all",
    ...
    dict(check="residue", sig="'Will be filled in", applies="all",
    ...
    # html-residue
    dict(check="html-residue", sig="title-block-header", applies="all",
    ...
    dict(check="html-residue", sig="CI runner path leaked into the HTML", applies="all",
    ...
    dict(check="image-policy", sig="per-image refusal cap", applies="all",
    ...
    dict(check="image-policy", sig="SVG contains <script>", applies="all",
    ...
    # pdf-cover
    dict(check="pdf-cover", sig="times on the PDF cover page", applies="all", requires="pdftotext",
    ...
    dict(check="pdf-cover", sig="leaked into the rendered PDF text", applies="all", requires="pdftotext",
    ...
    # schema-id
    dict(check="schema-id", sig="not valid JSON (", applies="all", requires="schemas",
    ...
    dict(check="schema-id", sig="a flattened path under the version", applies="all", requires="schemas",
    ...
    dict(check="schema-id", sig="An implementer following the $id", applies="all", requires="schemas",
    ...
    dict(check="schema-id", sig="disagrees with $id", applies="all", requires="schemas",
    ...
    # pdf-sync
    dict(check="pdf-sync", sig="pdftotext not on PATH", applies="all",
    ...
    dict(check="pdf-sync", sig="pdftotext failed", applies="all", requires="pdftotext",
         condition="pdftotext executes against the PDF",
         pulls="the pdftotext process outcome",
         compares_to="a clean execution"),
    dict(check="pdf-sync", sig="pdftotext could not read the PDF", applies="all", requires="pdftotext",
         condition="The PDF is machine-readable",
         pulls="pdftotext's exit status on the delivery PDF",
         compares_to="exit 0"),
    dict(check="pdf-sync", sig="does not contain the canonical this-stage base URL", applies="all", requires="pdftotext",
    ...
    dict(check="pdf-sync", sig="expected only as a previous-stage reference", applies="all", requires="pdftotext",
    ...
    dict(check="link-mismatch", sig="Visible link text and href disagree in the HTML", applies="all",
    ...
    # ref-rfc
    dict(check="ref-rfc", sig="links an RFC URL that names", applies="all",
    ...
    dict(check="ref-rfc", sig="says 'RFC ", applies="all",
    ...
    # boilerplate-dup
    dict(check="boilerplate-dup", sig="status paragraph appears", applies="all",
    ...
    # pdf-fonts
    dict(check="pdf-fonts", sig="pdffonts failed", applies="all", requires="pdffonts",
         condition="pdffonts executes against the PDF",
         pulls="the pdffonts process outcome",
         compares_to="a clean execution"),
    dict(check="pdf-fonts", sig="not declared by the package's own CSS", applies="all", requires="pdffonts",
    ...
    dict(check="manifest", sig="manifest.json is not valid JSON", applies="all", requires="manifest",
         ...
         compares_to="must parse without error"),
    dict(check="manifest", sig="manifest lists missing file", applies="all", requires="manifest",
    ...
    dict(check="manifest", sig="sha256 mismatch for", applies="all", requires="manifest",
    ...
    # junk-files
    dict(check="junk-files", sig="Working directory", applies="all",
    ...
    dict(check="junk-files", sig="Junk file in package", applies="all",
    ...
    # case
    dict(check="case", sig="Mixed-case filename", applies="all",
    ...
    dict(check="case", sig="Mixed-case path in docs.oasis-open.org URL", applies="all", sites=2,
    ...
    # symlinks
    dict(check="symlinks", sig="points at its own ancestor", applies="all",
    ...
    # dead-lists
    dict(check="dead-lists", sig="References mailing address", applies="all", sites=2,
    ...
    # vml-fallback
    dict(check="vml-fallback", sig="browsers that ignore VML show nothing", applies="all",
    ...
    # asset-refs
    dict(check="asset-refs", sig="which is not in the package; it 404s", applies="all",
...
def main() -> int:
    ...
    ap.add_argument("--emit-manifest", action="store_true",
                    help="write the two release manifests into the stage dir before "
                         "checking: manifest.json (sha256, source commit, tool versions) "
                         "and <stem>-manifest.txt, the Work Product Manifest File")
