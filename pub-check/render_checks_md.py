...
CLASS_DESCRIPTIONS = {
    ...
    "name-chars": 'Every filename and directory name must stay within the sixty-four permitted characters; UNDERSCORE is a BLOCKER in an identifying (document-URI) name and a non-blocking WARN elsewhere. An empty identifying name is a BLOCKER, not a silent pass.',
    "normdef-refs": 'Every shipped normative schema/grammar/code file (Standards Track) must be referenced from the Work Product (TC Process 2.2.5).',
    ...
    "xml-namespace": 'Every namespace a shipped .xsd/.wsdl/.rng declares as its own must be a docs.oasis-open.org/[tc-shortname]/ns/xxxx URI (consistent scheme) or an allowlisted urn:.',
