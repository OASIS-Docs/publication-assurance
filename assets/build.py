...
# ===========================================================================
# hero.svg (1200 x 250)  README masthead
# ===========================================================================
def build_hero():
    W, H = 1200, 250
    ...
    e.append(kicker(56, 62, "OASIS TC Administration"))
    e.append(text(56, 102, "OASIS Docs Publication Sandbox", 30, INK, 700))
    e.append(text(56, 132, "The Markdown to HTML to PDF publication pipeline, and the", 13.5, BODY))
    e.append(text(56, 152, "acceptance gate a TC can run before it submits.", 13.5, BODY))
    e.append(text(56, 196, "Michael Coletta  ·  Technical Advisor, OASIS Open", 11, MUTED))

    # right: mini pipeline + gate chip
    cw, ch = 96, 44
    gap = 42
    x0 = 742
    yc = 88
    for i, t in enumerate([".md", ".html", ".pdf"]):
        x = x0 + i * (cw + gap)
        e.append(rect(x, yc - ch / 2, cw, ch, SURFACE, BORDER2, HAIR))
        e.append(text(x + cw / 2, yc + 5, t, 15, INK, 500, anchor="middle", font=MONO))
        if i < 2:
            e.append(arrow(x + cw + 7, yc, x + cw + gap - 7, yc, ACCENT, 1.5, "aAcc"))

    gx, gy, gw, gh = x0, 132, cw * 3 + gap * 2, 44
    e.append(rect(gx, gy, gw, gh, GOOD_T, GOOD, EMPH))
    e.append(text(gx + gw / 2, gy + 27, "pub-check · 92 checks · exit 0 = publish",
...
def build_gate():
    ...
    e.append(text(56, 86, "pub-check: the publication gate", 26, INK, 700))
    ...
    for i, t in enumerate(["spec-vN.N-csdNN.md", "        + .html + .pdf",
                           "schema/*.json", "manifest.json"]):
        e.append(text(wx + 18, wy + 74 + i * 26, t, 11.5, BODY, 400, font=MONO))
    ...
    e.append(text(px + 24, py + 36, "92 checks in 34 classes", 16, INK, 700))
    ...
    groups = [
        ...
        ("Package hygiene", 10, "junk files, symlinks, schema $id, sha256"),
    ]
    ...
    e.append(text(px + 24, py + ph + 46,
                  "Track-aware: DOCX-native packages skip Markdown-only checks and gain "
                  "render-fidelity checks.", 11, MUTED))
...
def build_chain():
    ...
    nodes = [
        ("TC build", ["make / CI / editor tools", "renders md · html · pdf"], SURFACE, BORDER2, INK),
        ("manifest.json", ["sha256 + roles", "source commit · tools"], SURFACE, BORDER2, INK),
        ("pub-check", ["92 checks, before the vote", "TC side · exit 0"], ACC_T, ACCENT, ACCENT),
    ...
    for i, (name, lines, fill, stroke, tcol) in enumerate(nodes):
        ...
        mono = name in ("manifest.json", "pub-check")
...
def build_dovetail():
    ...
    e.append(text(bx + 46, 381, "RUN pub-check IN THE TC'S OWN CI", 13, INK, 500, ls=0.6))
    ...
    e.append(text(lcx, 560, "all 92 conditions: observed vs expected, in full", 11, BODY, anchor="middle"))
    ...
    e.append(text(lcx + 14, 716, "no review round-trip:", 10.5, MUTED))
    ...
    e.append(text(rcx, 369, "re-run pub-check, triage every finding:", 11.5, BODY, anchor="middle"))
    e.append(text(rcx, 387, "the whole 92-check validation layer plugs in here", 11.5, BODY, anchor="middle"))
    ...
    e.append(text(rbx, 469, "live ground truth pub-check never sees", 10.5, MUTED))
    ...
    e.append(text(ecx, ey + 20, "pub-check", 14.5, "#ffffff", 700, anchor="middle", font=MONO))
    e.append(text(ecx, ey + 52, "pub_check.py", 12, ACCENT, 500, anchor="middle", font=MONO))
    e.append(text(ecx, ey + 74, "92 individual checks", 11.5, BODY, anchor="middle"))
    e.append(text(ecx, ey + 92, "34 check classes", 11.5, BODY, anchor="middle"))
    ...
    # submitted package handoff: left lane -> across the top of the engine -> intake
...
def build_stack():
    ...
    e.append(text(450, L1Y + 25, "LAYER 1 · VALIDATION · pub-check (mechanical, tool-side)",
                  13.5, "#ffffff", 700, anchor="middle", ls=0.6))
    e.append(text(100, L1Y + 68, "pub_check.py: 92 checks across 34 classes", 13, INK, 700))
    ...
    e.append(text(100, L1Y + 194, "Output: a Validation Report, observed vs expected for all 92.",
                  11.5, ACCENT, 500))
    # annotation panel
    ...
    gates = [
        ("Step 4b: run pub-check + triage findings (the dovetail)", ACCENT, 500, ACCENT),
    ...
    e.append(text(100, L2Y + L2H - 14,
                  "The audit record carries 15 mandatory gates; every gate needs evidence.",
                  10.5, MUTED))
