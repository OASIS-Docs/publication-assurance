...
This directory contains the real quality record of a real OASIS
publication, reproduced verbatim as a worked example.
-->

# Worked example: OpenEoX EoX-Core v1.0 CSD01

The real Validation Report for a real publication: the
...
nothing here was produced specially for documentation.
...
Who produces what: running `oasis_pub_check.py` yourself gives you the
...
record filed to the ticket, not reproduced here.

The publication's history shows why the gate exists: the TC's first release
...
criteria grow as real correction rounds surface new failure modes (the
