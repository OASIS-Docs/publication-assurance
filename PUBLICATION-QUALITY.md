...
![How the two layers dovetail](assets/architecture/validation-audit-dovetail.svg)

## The two layers

**Layer 1 is validation.** A single Python file, [`pub-check/pub_check.py`](pub-check/),
runs 92 individual checks across 34 check classes against your package: the
files you are about to submit, exactly as you would submit them. It needs no
...
see: byte identity between the live site and the repository, the rendering
against your TC's own precedent, the chairs in the front matter against the
live roster, every directory index in the chain, all four announcement
channels at their destinations. An independent adversarial verifier, briefed
to refute the publication rather than confirm it, must pass before the audit
closes.

**The dovetail.** Step 4b of the intake checklist is: re-run pub-check, on
our side, with the identical code, and triage every finding. Your entire
92-check validation layer plugs into the audit as one step. That is the
...
![The OASIS publication quality stack](assets/architecture/two-layer-stack.svg)

## Layer 1: the 92 checks

Every check exists because a real publication went wrong in that specific
way: a `TODO: add the link.` that shipped and stayed live for years, a
lowercase citation that 404ed on the case-sensitive host, a PDF rendered
from an older draft than the HTML beside it, a self-referential symlink that
a deploy materialized into 41 nested directories. The gate is the accumulated
record of those correction rounds, plus the publication pipeline's own lint
registry, calibrated against a 13-package regression corpus of real
submissions in their original received form.

The checks group into six areas:

![pub-check validation flow](assets/gate.svg)
...
| Package hygiene | 10 | junk files, recursive symlinks, schema `$id` vs publish path, manifest sha256 |
...
The gate is track-aware. Markdown-authored packages get the markdown checks;
DOCX-native packages (KMIP and PKCS#11 style) skip those and gain Word
render-fidelity checks instead; packages authored in other formats
(DocBook/XML, LaTeX) receive the full format-agnostic output and package
suites. The bar is always the output: conformant HTML and PDF with the
...
Both layers produce a standard report, and both reports are filed to your
TCADMIN ticket and to the TC's `_audit/` record:

- **The Validation Report** itemizes all 92 conditions: the condition
...
conditions, with 9 warnings and 3 informational notes, every one triaged in
...
python3 pub-check/pub_check.py openeox/eox-core/v1.0/csd01/

# against a submission zip
python3 pub-check/pub_check.py eox-core-1.0-csd-01-20260713-rc3.zip

# machine-readable
python3 pub-check/pub_check.py <target> --json

# the check inventory, asserted from the code itself
python3 pub-check/pub_check.py --list-checks

# also write manifest.json (per-file sha256, source commit, tool versions)
python3 pub-check/pub_check.py <target> --emit-manifest
```
...
3. Ship the validation report (or the exit code) with your submission.
...
[OASIS-Docs/publication-sandbox](https://github.com/OASIS-Docs/publication-sandbox).
